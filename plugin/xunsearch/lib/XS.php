<?php
namespace Plugin\XunSearch\Lib;
...
class XSFieldMeta
{
    ...
    public function getCustomTokenizer()
    {
        if (isset(self::$_tokenizers[$this->tokenizer])) {
            ...
            // yanue changed to namespace;
            $name = __NAMESPACE__ . '\XSTokenizerFull';
    ...
    private function initHighlight()
    {
        ...
        foreach ($terms as $term) {
            if (!preg_match('/[a-zA-Z]/', $term)) {
                $pairs[$term] = '<em class="highlight">' . $term . '</em>';
            } else {
                $pattern[] = '/' . strtr($term, array('+' => '\\+', '/' => '\\/')) . '/i';
                $replace[] = '<em class="highlight">$0</em>';
            }
        }
